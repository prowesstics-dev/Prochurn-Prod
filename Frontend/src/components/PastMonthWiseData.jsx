...
import { pastmonthdataview, pastmonthdatadownloadview } from "../api.js";
// import Sidebar from "./SidebarUpload.jsx";
import styles from "./PastMonthWiseData.module.css";
import Swal from "sweetalert2";
import { FaSignOutAlt, FaUserCircle, FaFileDownload } from "react-icons/fa";

const PastMonthWiseData = () => {
  ...
  const handleDownload = () => {
    ...
    pastmonthdatadownloadview(month)
      .then(() => {
                  ...
                  {data.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      {columns.map((col, colIndex) => (
                        <td key={colIndex}>
                        {['policy start date_YEAR', 'policy end date_YEAR'].includes(col)
                          ? String(row[col])
                          : row[col] ?? "—"}
                      </td>
                      ))}
...
export default PastMonthWiseData;