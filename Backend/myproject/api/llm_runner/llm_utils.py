...
def user_wants_chart(question: str) -> bool:
    ...
    keywords = [
        ...
        "trend", "distribution", "comparison", "compare",
...
def llm_generate_chart_config(question, rows):
    try:
        ...
        sanitized_rows = sanitize_for_json(rows[:100])
...
def generate_chart_and_response(question, rows):
    ...
    # ✅ Only generate if user asked for a chart/visualization
    if not user_wants_chart(question):
        print("⚡ Skipping chart: user did not request one.")
        return None, rows

    try:
        # chart_config = llm_generate_chart_config(question, rows)
        chart_config = None
        if user_wants_chart(question):   # ✅ only if user explicitly asked
            chart_config = llm_generate_chart_config(question, rows)
        else:
            print("⚡ Skipping chart: user did not request visualization.")
        print("📊 Chart config:", chart_config)
    except Exception as chart_err:
        print("⚠️ Chart generation failed:", chart_err)
        return None, rows
